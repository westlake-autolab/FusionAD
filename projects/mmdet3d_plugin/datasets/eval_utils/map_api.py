...
class NuScenesMap:
    ...
    def __init__(self,
        ...
        # Parse the map version and logging.info an error for deprecated maps.
...
class NuScenesMapExplorer:
    ...
    def render_map_in_image(self,
        ...
        if verbose:
            logging.info('Warning: Note that the projections are not always accurate as the localization is in 2d.')
    ...
    def render_egoposes_on_fancy_map(self,
        ...
        map_poses = []
        if verbose:
            logging.info('Adding ego poses to map...')
        for scene_token in tqdm(scene_tokens_location, disable=not verbose):
            ...
            if verbose and scene_id in scene_blacklist:
                logging.info('Warning: %s is known to have a bad fit between ego pose and map.' % scene_name)
