...
def load_prediction(result_path: str, max_boxes_per_sample: int, box_cls, verbose: bool = False, category_convert_type='detection_category') \
    ...
    :param verbose: Whether to logging.info messages to stdout.
...
def accumulate_Hungarian(gt_boxes: EvalBoxes,
    ...
    :param verbose: If true, logging.info debug messages.
    ...
    for box in pred_boxes.all:
         if box.sample_token in sample_token_2_pred_agents.keys():
             sample_token_2_pred_agents[box.sample_token].append(box)
         else:
             sample_token_2_pred_agents[box.sample_token] = [box]
    ...
    # ---------------------------------------------
    import pdb
    for sample_token in tqdm.tqdm(gt_boxes.sample_tokens):
        if sample_token not in sample_token_2_pred_agents:
            break
        pred_agents = sample_token_2_pred_agents[sample_token]
        gt_agents = gt_boxes[sample_token]
        if len(gt_agents) > 0:
            ...
            for i in range(len(pred_agents)):
                cost_matrix[i] = get_distances(np.array(pred_agents[i].translation), gt_translations)
                cost_matrix[i][np.nonzero(cost_matrix[i] > dist_th)] = 10000.0

            r_list, c_list = linear_sum_assignment(cost_matrix)

            for i in range(len(r_list)):
                if cost_matrix[r_list[i], c_list[i]] <= dist_th:
