...
from .eval_utils import load_prediction, load_gt, accumulate, accumulate_motion, \
    DetectionMotionBox, DetectionMotionBox_modified, \
    DetectionMotionMetrics, DetectionMotionMetricDataList, accumulate_Hungarian
...
def class_tp_curve(md_list: DetectionMetricDataList,
    ...
    for metric in TP_METRICS:
        ...
        if metric == 'trans_err':
            ...
            logging.info(f'Recall: {detection_name}: {md.max_recall_ind/100}')
...
def filter_eval_boxes_by_id(nusc: NuScenes,
    ...
    :param verbose: Whether to logging.info to stdout.
    """
    ...
    if verbose:
        logging.info("=> Original number of boxes: %d" % total)
        logging.info("=> After anns based filtering: %d" % anns_filter)
...
def filter_eval_boxes_by_visibility(
    ...
    if verbose:
        logging.info("=> Original number of boxes: %d" % total)
        logging.info("=> After visibility based filtering: %d" % anns_filter)
...
class MotionEval(NuScenesEval):
    ...
    def evaluate(self) -> Tuple[DetectionMotionMetrics,
        ...
        # logging.info(self.cfg.dist_fcn_callable, self.cfg.dist_ths)
        ...
        # print(traj_metrics)
        print("min_ade_err:", traj_metrics['car']['min_ade_err'])
        print("min_fde_err:", traj_metrics['car']['min_fde_err'])
        print("miss_rate_err:", traj_metrics['car']['miss_rate_err'])
        ...
        for class_name in self.cfg.class_names:
            ...
            for dist_th in self.cfg.dist_ths:
                ...
                ap = calc_ap(
                    ...
                    self.cfg.min_precision)
                metrics.add_label_ap(class_name, dist_th, ap)
                # print("minfde-ap: ", class_name, dist_th, ap)
            # Compute TP metrics.
            for metric_name in TP_METRICS:
                metric_data = metric_data_list[(
                    class_name, self.cfg.dist_th_tp)]
    ...
    def main(self,
        ...
        logging.info('Object Class\tAP\tATE\tASE\tAOE\tAVE\tAAE')
        class_aps = metrics_summary['mean_dist_aps']
        class_tps = metrics_summary['label_tp_errors']
        for class_name in class_aps.keys():
            logging.info('%s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f'
    ...
    def render(self, metrics: DetectionMetrics,
        ...
        """
        if self.verbose:
            logging.info('Rendering PR and TP curves')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataroot', type=str, default='UniAD/data/nuscenes',
    ...
    parser.add_argument('--verbose', type=int, default=1,
                        help='Whether to logging.info to stdout.')
