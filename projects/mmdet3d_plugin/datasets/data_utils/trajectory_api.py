...
class NuScenesTraj(object):
    ...
    def get_sdc_planning_label(self, sample_token):
        sd_rec = self.nusc.get('sample', sample_token)
        prev_sd_rec = self.nusc.get('sample', sample_token)
        ...
        for _ in range(self.planning_steps):
            prev_annotation_token = prev_sd_rec['prev']
            if prev_annotation_token == '':
                break
            prev_sd_rec = self.nusc.get('sample', prev_annotation_token)
            l2e_r_mat_curr, l2e_t_curr, e2g_r_mat_curr, e2g_t_curr = self.get_l2g_transform(
                prev_sd_rec)  # (lidar to global at current frame)

            # bbox of sdc under current lidar frame
            prev_bbox3d = self.generate_sdc_info(self.sdc_vel_info[prev_annotation_token], as_lidar_instance3d_box=True)
            ...
            past_planning.append(prev_bbox3d)
        ...
        past_planning_all = np.zeros((1, self.planning_steps, 3))
        n_valid_timestep = len(past_planning)
        if n_valid_timestep > 0:
            past_planning = [p.tensor.squeeze(0) for p in past_planning]
            past_planning = np.stack(past_planning, axis=0)  # (valid_t, 9)
            past_planning = past_planning[:, [0, 1, 6]]  # (x, y, yaw)
            past_planning_all[:, :n_valid_timestep, :] = past_planning
        ...
        return {'planning':planning_all,'past_planning':past_planning_all}, planning_mask_all, command