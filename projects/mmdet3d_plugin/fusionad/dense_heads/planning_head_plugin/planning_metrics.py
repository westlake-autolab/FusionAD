...
class PlanningMetric(Metric):
    def __init__(
        ...
        self.add_state("total_obj_col", default=torch.zeros(self.n_future), dist_reduce_fx='sum')
    ...
    def update(self, trajs, gt_trajs, gt_trajs_mask, segmentation):
        ...
        obs_box_coll_all_time_sum = torch.ones(self.n_future).to(obj_box_coll_sum.device)
        for i in range(self.n_future):
            if obj_box_coll_sum[i] == 0:
                obs_box_coll_all_time_sum[i] = 0
            else:
                break

        self.obj_col += obj_coll_sum
        self.obj_box_col += obj_box_coll_sum
        self.total_obj_col += obs_box_coll_all_time_sum
    ...
    def compute(self):
        ...
        'total_time_box_col': self.total_obj_col / self.total,
        }
        return res
