...
class SamplingResult_segformer(util_mixins.NiceRepr):
    ...
    def __init__(self, pos_inds, neg_inds, bboxes, gt_bboxes, gt_masks,assign_result,
        ...
        if gt_bboxes.numel() == 0:
            ...
            #logging.info('pos_gt_bboxes',self.pos_gt_bboxes.shape)
            #logging.info('gt_mask',gt_masks.shape)
    ...
    def to(self, device):
        ...
        Example:
            ...
            >>> logging.info(f'self = {self.to(None)}')
            >>> # xdoctest: +REQUIRES(--gpu)
            >>> logging.info(f'self = {self.to(0)}')
        """
        _dict = self.__dict__
    ...
    def assign(self,
        ...
        for i in range(min(self.max_pos, 300//num_gts)):
            ...
            matched_col_inds = torch.from_numpy(matched_col_inds).to(
                bbox_pred.device)     
            #logging.info(matched_row_inds)
                
            cost[matched_row_inds,:] = INF   
            #index_set.(matched_row_inds)
            #logging.info('this mathed row inds ', len(matched_row_inds), i)
...
class HungarianAssigner_multi_info(BaseAssigner):
    ...
    def assign(self,
        ...
        assert gt_bboxes_ignore is None, \
            'Only case when gt_bboxes_ignore is None is supported.'
        #logging.info(bbox_pred.shape, cls_pred.shape,mask_pred.shape,gt_bboxes.shape,gt_labels.shape,gt_mask.shape)
        num_gts, num_bboxes = gt_bboxes.size(0), bbox_pred.size(0)
